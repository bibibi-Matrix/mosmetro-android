...
public final class PermissionUtils {
    ...
    public boolean isCoarseLocationGranted() {
        // On API 33+ SSID access is also granted via NEARBY_WIFI_DEVICES
        if (Build.VERSION.SDK_INT >= 33 && isNearbyWifiDevicesGranted()) return true;
    ...
    @RequiresApi(33)
    public boolean isNearbyWifiDevicesGranted() {
        int p = ContextCompat.checkSelfPermission(context, Manifest.permission.NEARBY_WIFI_DEVICES);
    ...
    /**
     * Requests all permissions required to obtain the real SSID:
     * location pair on API 23-32, NEARBY_WIFI_DEVICES additionally on API 33+.
     */
    @RequiresApi(23)
