...
public class Notify extends NotificationCompat.Builder {
    ...
    public Notify(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= 26) {
            int importance;
            switch (Util.getIntPreference(context, "pref_notify_priority", 0)) {
                case -2: importance = NotificationManager.IMPORTANCE_MIN; break;
                case -1: importance = NotificationManager.IMPORTANCE_LOW; break;
                case 1:
                case 2: importance = NotificationManager.IMPORTANCE_HIGH; break;
                default: importance = NotificationManager.IMPORTANCE_DEFAULT;
            }

            NotificationChannel channel = new NotificationChannel(
                    CHANNEL_ID, context.getString(pw.thedrhax.mosmetro.R.string.app_name),
                    importance
            );
        ...
        // without an icon, so make sure a valid icon is always present
        icon(pw.thedrhax.mosmetro.R.drawable.ic_notification_connecting_colored,
