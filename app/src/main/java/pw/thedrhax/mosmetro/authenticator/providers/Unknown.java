    ...
    public Unknown(final Context context, final HttpResponse response) {
        ...
        add(new InitialConnectionCheckTask(this, response) {
            ...
            public boolean handle_response(HashMap<String, Object> vars, HttpResponse response) {
                ...
                // No redirect found: the response is a final page, not a portal

                if (Boolean.TRUE.equals(vars.get("midsession"))) {
                    // False negative of generate_204 (e.g. a single blocked
                    // endpoint): there is nothing to solve here
                    Logger.log(Logger.LEVEL.DEBUG,
                            "Midsession | No redirect in response, nothing to solve");
                ...
                if (code >= 200 && code < 300) {
                    // The page loaded successfully, so the Internet
                    // connection is most probably already available
                    Logger.log(Logger.LEVEL.DEBUG,
                            "Internet connection is already available");
                    vars.put("result", RESULT.ALREADY_CONNECTED);
                    return false;
                }

                // Redirect or error without Location header:
