...
/**
 * The ByFly class implements support for the public Wi-Fi networks
 * "BELTELECOM" and "byfly WIFI" in Belarus (Beltelecom captive portal,
 * ciscowifi.beltelecom.by).
 *
 * The portal requires login and password which are stored in the
 * "pref_byfly_credentials" preference. The "safe=1" cookie is required
 * by the portal to skip the interactive landing page.
 *
 * Detection: redirect contains "ciscowifi.beltelecom.by".
 *
 * Based on the implementation from XeonDead/mosmetro-android.
 *
 * @see Provider
 */

public class ByFly extends Provider {
    private static final String TAG = "ByFly";
    private static final String CREDS_KEY = "pref_byfly_credentials";
    private static final String PORTAL = "https://ciscowifi.beltelecom.by";

    private String redirect = PORTAL;
    ...
    public ByFly(Context context, HttpResponse res) {
         ...
         * ⇐ Location redirect: https://ciscowifi.beltelecom.by/login > redirect
         */
        add(new InitialConnectionCheckTask(this, res) {
            ...
            public boolean handle_response(HashMap<String, Object> vars, HttpResponse response) {
                String found = response.parseAnyRedirectOrNull();

                if (found != null) {
                    redirect = found;
                } else {
                    Logger.log(Logger.LEVEL.DEBUG,
                            TAG + " | Redirect not found in response, using default");
                }
        ...
        /**
         * Fetching the auth page
         * ⇒ GET https://ciscowifi.beltelecom.by/login < redirect + cookie safe=1
         * ⇐ HTML with form.login-pass-form > form
         */
        add(new NamedTask(context.getString(R.string.auth_auth_page)) {
            ...
            public boolean run(HashMap<String, Object> vars) {
                ...
                try {
                    ...
                    Logger.log(Logger.LEVEL.DEBUG,
                            TAG + " | Auth page: " + res.getResponseCode());
                ...
                if (forms.size() == 0) {
                    Logger.log(Logger.LEVEL.DEBUG, TAG + " | " + res.toBodyString());
                    Logger.log(context.getString(R.string.error,
        ...
        /**
         * Filling and sending the auth form
        ...
        add(new NamedTask(context.getString(R.string.auth_auth_form)) {
            ...
            public boolean run(HashMap<String, Object> vars) {
                ...
                try {
                    HttpResponse res = client.submit(form).retry().execute();
                    Logger.log(Logger.LEVEL.DEBUG, TAG + " | " + res.toBodyString());
                } catch (IOException ex) {
                    Logger.log(Logger.LEVEL.DEBUG, ex);
                    Logger.log(context.getString(R.string.error,
    ...
    public static boolean match(HttpResponse response) {
        ...
        return redirect != null && redirect.contains("ciscowifi.beltelecom.by");
    }
}
